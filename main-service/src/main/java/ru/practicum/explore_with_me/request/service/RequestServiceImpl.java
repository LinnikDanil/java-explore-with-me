...
public class RequestServiceImpl implements RequestService {
    ...
    public ParticipationRequestDto createRequest(Long userId, Long eventId) {
        ...
        requestRepository.findByRequesterIdAndEventId(userId, eventId)
                .ifPresent(e -> {
                    throw new AlreadyExistEwmException(String.format("Request for event id = %d and user id = %d already exists", eventId, userId));
                });

        validateEvent(userId, eventId, event);

        long currentParticipants = requestRepository.countAllByEventIdAndStatusEquals(eventId, RequestStatuses.CONFIRMED);
    ...
    private void validateEvent(Long userId, Long eventId, Event event) {
        if (userId.equals(event.getInitiator().getId())) {
            throw new ForbiddenActionEwmException(
                    String.format("Owner id = %d cannot submit a request to participate in their event id = %d", userId, eventId));
        }
